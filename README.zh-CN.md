...
Smart Harbor 是一个面向自托管服务的导航主页。它会根据你当前的网络环境，在局域网和公网地址之间自动切换，让同一套书签在家里、移动网络或外部网络下都能直接访问。

## 核心特性

- **局域网 / 公网自动路由**：根据当前网络环境自动切换内网地址和外网地址
...
这个项目在 OpenAI Codex 和 Claude 的支持下完成，并采用 vibe coding 的方式进行快速迭代、实现与打磨。
